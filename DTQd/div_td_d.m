function c = div_td_d(a,b)
% division of  triple - double  and double  the result is triple-double
%
% input  a   triple-double  but in vector form
%        b   double
% output c   triple-double  but in vector form
% written by P. B. Du & H. Jiang NUDT 07/01/2013
% 似乎sloppy和accurate版本一样。

if ~isa(b,'double')
    error('wrong  b has to be in double format');
end

%  approximate quotient 
q0=a(1)/b;

% Compute the remainder  a - q0 * b 
[t0,t1]=TwoProduct(q0,b);
r=TD(a)-DD(t0,t1);            % r is in quad-format   change a into quad-double format  

% Compute the first correction 
q1=part1(r)/b;
[t0,t1]=TwoProduct(q1,b);
r=r-DD(t0,t1);                % using  minus in quad double format

% Second correction to the quotient.

q2=part1(r)/b;
%或者这里就直接
%[c(1),c(2),c(3)] = quick_renorm(q0, q1, q2);
%则即为sloppy版本。

[t0,t1]=TwoProduct(q2,b);
r=r-DD(t0,t1);  

% Final correction to the quotient. 

 q3 = part1(r)/ b;

[c(1),c(2),c(3)] = quick_renorm(q0, q1, q2, q3);

end